...
def denoise_corrupted_batched(corrupted_img, model, scheduler, num_samples=10, batch_size=None, t_start=None, device=None):
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_gen_images", type=int, default=2, help="Number of images to generate")
    ...
    all_last_latents = [[] for _ in range(num_gen_images)]  # list of lists: [num_gen_images][timesteps][num_samples, C, H, W]
    for t_idx, timestep in enumerate(tqdm(timesteps, desc="Denoising latents for all images")):
        ...
        for img_idx in range(num_gen_images):
            denoised = denoise_corrupted_batched(
                latents_at_t[img_idx], model, scheduler, 
                num_samples=num_denoised_samples, batch_size=batch_size, t_start=timestep, device=device
            )
    ...
    print(f"📊 Computing original reward, IR, and gender scores for all {num_gen_images} generated images...")
    original_rewards, original_scores = reward_function(next_latents[:, -1])  # next_latents[:, -1] shape: [num_gen_images, C, H, W]
    original_ir_person = original_scores['ir_person']
    original_sex_score_binary = original_scores['sex_score_binary']

    # For each timestep, aggregate statistics (mean of stds, mean of means, etc.) across all images for that timestep
    reward_stds = []
    reward_means = []
    reward_mean_diffs = []
    ir_person_means = []
    ...
    for t_idx, timestep in enumerate(tqdm(timesteps, desc="Evaluating rewards by timestep")):
        ...
        for img_idx in range(num_gen_images):
            ...
            per_image_reward_mean_diffs.append(abs(rewards.mean().item() - original_rewards[img_idx].item()))
            ...
            per_image_ir_mean_diffs.append(abs(ir_person.mean().item() - original_ir_person[img_idx].item()))
            
            per_image_gender_means.append(sex_score_binary.mean().item())
            per_image_gender_stds.append(sex_score_binary.std().item())
            per_image_gender_mean_diffs.append(abs(sex_score_binary.mean().item() - original_sex_score_binary[img_idx].item()))
    ...
    print(f"💾 Saved: {os.path.join(plot_dir, 'timestep_vs_mean_gender_score_diff_from_original.png')}")
    plt.close()
