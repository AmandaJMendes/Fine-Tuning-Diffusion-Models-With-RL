...
def get_images_from_wandb(run_path, out_dir="frames", history_key="eval/samples"):
    ...
    for row in tqdm.tqdm(run.scan_history(keys=["_step", history_key]), desc="steps"):
        step = row["_step"]
        ...
        for idx, rel in enumerate(filenames):  # already ordered!
            local = run.file(rel).download(
                root=out_dir, replace=False, exist_ok=True
...
def sample_and_plot_grid(samples_dict, k=5, out_dir="frames", plot_name="evolution_grid.png"):
    ...
    sorted_steps = [
        i for i in sorted(samples_dict.keys()) if i % 100 == 0
    ]  # sorted(samples_dict.keys())[:10] #
...
if __name__ == "__main__":
    ...
    ap.add_argument("--run_path", help="ENTITY/PROJECT/RUN_ID (for W&B runs)")
    ap.add_argument("--frames_dir", help="Path to local folder containing renamed frames")
    ap.add_argument("--out_dir", default="frames")
    ap.add_argument("--gif_name", default="samples.gif")
    ap.add_argument("--fps", type=int, default=1)
    ap.add_argument("--make_grid", action="store_true", help="Create evolution grid plot")
    ap.add_argument("--make_gif", action="store_true", help="Create GIF animation")
    ap.add_argument(
        "--k_images", type=int, default=5, help="Number of images to track in evolution grid"
    )
    ap.add_argument("--plot_name", default="evolution_grid.png")
    ap.add_argument(
        "--local_only", action="store_true", help="Work with local frames only (no W&B)"
    )
    ...
    if args.local_only:
        ...
        samples = get_images_from_wandb(args.run_path, out_dir=args.out_dir)
    ...
    if args.make_grid:
        sample_and_plot_grid(
            samples, k=args.k_images, out_dir=args.out_dir, plot_name=args.plot_name
        )

    if args.make_gif:
        make_gif(samples, out_dir=args.out_dir, gif_name=args.gif_name, fps=args.fps)
