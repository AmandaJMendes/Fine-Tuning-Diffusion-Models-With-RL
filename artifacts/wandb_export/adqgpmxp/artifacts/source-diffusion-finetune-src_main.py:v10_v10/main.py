...
if __name__ == "__main__":
    ...
    for epoch in tqdm(range(args.full_epochs), desc="Training Epochs", disable=not accelerator.is_main_process):
        ...
        for inner_epoch in range(args.epochs_per_sampling):
            ...
            # Convert timesteps to tensor for gathering across processes (all processes need to participate)
            timesteps_tensor = torch.tensor(train_timesteps, device=device)
            # Gather timesteps from all processes
            all_timesteps = accelerator.gather(timesteps_tensor)
            
            # Only log on main process
            if accelerator.is_main_process:
                # Convert back to list and log to wandb
                all_timesteps_lists = all_timesteps.cpu().tolist()
                print(f"all_timesteps_lists: {all_timesteps_lists}")
                tbl = wandb.Table(columns=["timestep"], data=[[t] for t in all_timesteps_lists])
                accelerator.log({f"timesteps/epoch_{epoch}_inner_{inner_epoch}": tbl}, step=global_step)
                # accelerator.log({
                #      f"timesteps/epoch_{epoch}_inner_{inner_epoch}": all_timesteps_lists,
                #  }, step=global_step)
